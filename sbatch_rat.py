...
def run_single_rat_sbatch(i_rat, lf_ratio, hf_ratio):
    msg = f"i_rat: {i_rat}, lf ratio: {lf_ratio:.2f}, hf ratio: {hf_ratio:.2f}"
    print(msg)
    te_fnc(i_rat, lf_ratio, hf_ratio)


def run_all_rats_desktop(lf_ratio, hf_ratio):
...
if __name__ == '__main__':
    if len(sys.argv) == 4:
        ...
        run_single_rat_sbatch(i_rat, lf_ratio, hf_ratio)
    elif len(sys.argv) == 3:
        print('Running all rat analyses without sbatch')
        lf_ratio = float(sys.argv[1])
        hf_ratio = float(sys.argv[2])
        run_all_rats_desktop(lf_ratio, hf_ratio)
    else:
        print('Not a valid number of args')
